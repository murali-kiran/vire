...
public class SocialPostRetrievalRepository {
    ...
    private static final String BASIC_QUERY = "SELECT s.* FROM t_social s " +
            ...
            "AND s.category_id IN (select social_category_master_id from social_category_master where is_personal=1 and category in (%s))";
    private static final String COMMON_WHERE = " ( sst.`type` ='%s' AND sst.value='%s' )";

    private static final String SEND_TO_TYPE_LOCATION = "Location";
    ...
    public List<SocialDao> getSocialListBySearch(Long profileId) {
    ...
    private String emergencyCode(ProfileDao profileDao) {
        ...
        var addressQuery = frameAddressQuery(emergencyCategoryId, profileDao.getPersonalProfile().getPermanentAddress());
    ...
    private String employmentCode(ProfileDao profileDao) {
        ...
        var addressQuery = frameAddressQuery(emergencyCategoryId, profileDao.getPersonalProfile().getPresentAddress());
    ...
    private StringBuffer frameAddressQuery(String categoryId, AddressDao address) {
        ...
        sb.append(" WHERE");
        sb.append(String.format(COMMON_WHERE, SEND_TO_TYPE_LOCATION, "all"));

        if (!StringUtils.isEmpty(city)) {
            sb.append(" OR");
            sb.append(String.format(COMMON_WHERE, SEND_TO_TYPE_LOCATION, city));
        }

        if (!StringUtils.isEmpty(district)) {
            sb.append(" OR");
            sb.append(String.format(COMMON_WHERE, SEND_TO_TYPE_LOCATION, district));
        }

        if (!StringUtils.isEmpty(state)) {
            sb.append(" OR");
            sb.append(String.format(COMMON_WHERE, SEND_TO_TYPE_LOCATION, state));
        }
