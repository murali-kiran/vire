...
@Slf4j
@Component
public class SocialPostFilterRetrievalRepository {
    ...
    public List<SocialDao> getSocialListByFilters(Long profileId, int pageNumber, int pageSize,
                                                 List<String> categoryFilters, List<String> communityIdFilters) {
        ProfileDao profileDao = profileRepositoryJpa.findById(profileId).get();
        String query = new String();
        if (Objects.nonNull(profileDao) && (categoryFilters != null || communityIdFilters != null)) {
            if (profileDao.getPersonalProfile() != null) {
                if (categoryFilters != null && communityIdFilters != null) {
                    query = "select a.* from (" +
                            " SELECT s.* FROM t_social s JOIN t_social_post_send_to sst ON s.social_id = sst.social_id  " +
                            " WHERE  ( sst.`type` ='community' AND sst.value IN ( " + String.join(", ", communityIdFilters) + " ) )" +
                            " UNION " +
                            "SELECT s.* FROM t_social s where s.category_id in (" + String.join(", ", categoryFilters) + ")" +
                            ") as a";
                }else  if (categoryFilters == null && communityIdFilters != null) {
                    query = "select a.* from (" +
                            " SELECT s.* FROM t_social s JOIN t_social_post_send_to sst ON s.social_id = sst.social_id  " +
                            " WHERE  ( sst.`type` ='community' AND sst.value IN ( " + String.join(", ", communityIdFilters) + " ) )" +
                           ") as a";
            ...
            var queryObject = entityManager.createNativeQuery(query, SocialDao.class);
            queryObject.setFirstResult((pageNumber - 1) * pageSize);
            queryObject.setMaxResults(pageSize);
            return queryObject.getResultList();
        }
