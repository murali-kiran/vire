...
@Slf4j
@Component
public class FeedPostFilterRetrievalRepository {
    ...
    public List<FeedsDao> getFeedListBySearch(Long profileId, List<String> communitiesList, Boolean sendToFollowers, int pageNumber, int pageSize) {
...
//        var communityIdFilters = communityProfileRepositoryJpa.findAllByProfileId(profileId).stream().map(c -> c.getCommunityId()+"").collect(Collectors.toList());
        ...
        if (Objects.nonNull(profileDao)) {
                query.append("SELECT feed.* FROM (" );
                //query.append("SELECT s.* FROM t_feeds s where s.profile_id="+profileId+" UNION ");
                query.append(communityFollowersCode(profileDao, communitiesList, sendToFollowers));
    ...
    private String communityFollowersCode(ProfileDao profileDao, List<String> communityIdFilters, Boolean sendToFollowers) {
       ...
       // communityChannelQuery.append(" JOIN ( ").append(communityQuery).append(" ) as community on address.feed_id = community.feed_id ");
        communityChannelQuery.append(communityQuery).append("  ");
